...
TEST_CASE("Interval rotation rounding", "[interval][matrix]")
{
    ...
    MatrixMax3<Interval> R = posei.construct_transformation_matrix();
    MatrixMax3<Interval> expected_R;
    expected_R.resize(2, 2);
    //@javidf: make sure the transformation values are correctly used for a 2D case: (cos(thete), -sin(theta); sin(theta), cos(theta))
    expected_R.row(0) << posei.transform(0, 0), posei.transform(0, 1);
    expected_R.row(1) << posei.transform(1, 0), posei.transform(1, 1);
