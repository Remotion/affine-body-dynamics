...
TEST_CASE("Rigid edge-vertex time of impact", "[ccd][rigid_toi][edge_vertex]")
{
    ...
    Pose<double> bodyA_pose_t0 = Pose<double>::Eye(dim);
    bodyA_pose_t0.position = Eigen::Vector3d(0, 0.5, 0).head(pos_ndof);
    // (
    //    Eigen::Vector3d(0, 0.5, 0).head(pos_ndof),
    //    Eigen::Vector3d(0, 0, 0).head(rot_ndof));
    Pose<double> bodyB_pose_t0 = Pose<double>::Eye(dim);
    Pose<double> bodyA_pose_t1 = Pose<double>::Eye(dim);
    Pose<double> bodyB_pose_t1 = Pose<double>::Eye(dim);
    ...
    SECTION("Rotation")
    {
        ...
        bodyA_pose_t1.transform = Eigen::Vector3d(0, 0, theta).tail(rot_ndof);
    }
...
TEST_CASE("Fast EE case", "[!benchmark][ccd][rigid_toi][edge_edge][fast]")
{
    ...
    MatrixMax3d trans = construct_rotation_matrix<double>(Eigen::Vector3d(1.56635729875648, 0.00484560212950007, -0.00477941835507233));
    Pose<double> bodyA_pose_t0(
        Eigen::Vector3d(-0.0743518262648221, 2.0045466941596, -7.30362621222097e-05),
        trans);
    trans = construct_rotation_matrix<double>(Eigen::Vector3d(1.56635729875648, 0.00484560212950007, -0.00477941835507233));
    Pose<double> bodyA_pose_t1(
        Eigen::Vector3d(-0.0743518262648221, 2.0045466941596, -7.30362621222097e-05),
        trans
    );
    trans = construct_rotation_matrix<double>(Eigen::Vector3d(-0.0330556101141157, -0.0327790458706025, 1.56913520538901));
    Pose<double> bodyB_pose_t0(
        Eigen::Vector3d(-0.0625913211582116, 9.56420995832003, -0.0276191274028154),
        trans
    );
    trans = construct_rotation_matrix<double>(Eigen::Vector3d(-0.0467509876415133, -0.0464937186371367, 1.56888122389168));
