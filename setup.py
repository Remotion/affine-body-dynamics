...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        extdir = os.path.join(os.path.abspath(os.path.dirname(
            self.get_ext_fullpath(ext.name))), "abd")

        cmake_args = ["-DCMAKE_LIBRARY_OUTPUT_DIRECTORY=" + extdir,
                      "-DPYTHON_EXECUTABLE=" + sys.executable,
                      "-DABD_WITH_UNIT_TESTS=OFF",
                      "-DABD_WITH_CODE_COVERAGE=OFF",
                      "-DABD_WITH_DERIVATIVE_CHECK=OFF",
                      "-DABD_WITH_PROFILING=OFF",
...
setup(
    ...
    author_email="",
    description="ABD Python Bindings",
    long_description=long_description,
    long_description_content_type="text/markdown",
    url="",
    ext_modules=[CMakeExtension("abd")],
