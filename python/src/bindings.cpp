...
PYBIND11_MODULE(rigidipc, m)
{
    ...
    m.def(
        ...
        py::arg("out_dir"));
    // @javidf: all "rotation"s in Pose are changed to "transform" to be consistent with ABD formulations. The inputs are still "rotation"
    py::class_<PoseD>(m, "Pose")
        ...
        .def_property(
            "rotation",
            [](PoseD& self) -> VectorMax3d& { return self.transform; },
